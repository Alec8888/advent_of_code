...
def show_anti_nodes_on_grid(grid, anti_nodes):
    for i, row in enumerate(grid):
        for j, val in enumerate(row):
...
def find_anti_nodes(grid, char_pos_list):
    ...
    for i, pos in enumerate(char_pos_list):
        for j in range(i + 1, len(char_pos_list)):
            ...
            # find anti nodes on this line, check both dir on line, is it in bounds?
            antinode1 = (pos1[0] - run, pos1[1] - rise)
            antinode2 = (pos2[0] + run, pos2[1] + rise)
...
# for each char in char_pos_map, find the antinodes 
anti_nodes = set()
